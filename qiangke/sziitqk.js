...
doc.onload = function(){
	...
	setTimeout(function(){
		...
		{
			var Tds = docc.getElementById("table2").getElementsByTagName("table")[1].getElementsByTagName("td");
			var bsubmit = false;
			for(var k=0;k<number.length;k++){
				var n = 1;
				for(var i = 11; i < Tds.length;i += 18)
				{
						if(number[k] > 0 && Tds[i].innerText.indexOf(number[k]) >= 0 && parseInt(Tds[i - 4].innerText) > 0)
						{
								docc.getElementById("DataGrid1__ctl" + n +"_zhj1").checked = true;
								number[k]=0;
								bsubmit = true;
								docc.getElementById("Button1").click();
								return true;
...
{
	var Tds = document.getElementById("table2").getElementsByTagName("table")[1].getElementsByTagName("td");
	var bsubmit = false;
	for(var k=0;k<number.length;k++){
		var n = 1;
		for(var i = 11; i < Tds.length;i += 18)
		{
				if(number[k] > 0 && Tds[i].innerText.indexOf(number[k]) >= 0 && parseInt(Tds[i - 4].innerText) > 0)
				{
						document.getElementById("DataGrid1__ctl" + n +"_zhj1").checked = true;
						number[k]=0;
						bsubmit = true;
